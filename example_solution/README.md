* The archive must directly contain the source code files, without any nested
  folders.

* `Makefile`, `test.c`, `checker.py` - all that is going to be ignored anyway,
...
- +5 баллов: каждая из N корутин получает T / N микросекунд, где
  T - target latency введенное как параметр командной строки `-l`,
  вот так:
  ...
  Например:
    ...
    -l 0 # Yield всегда.
    -l 10 # Yield не чаще, чем раз в 10 микросек.
    -l 120 # Yield не чаще, чем раз в 120 микросек.

  Для парсинга аргументов необходимо использовать стандартную
  функцию getopt().

  После каждой итерации сортировки вы делаете yield только если
  квант времени текущей корутины закончился.

- +5 баллов: позволить задавать количество корутин через параметр
  командной строки `-n`, вот так:

    -n <число > 0>

  Например:

    -n 1 # Одна корутина на все файлы.
    -n 5 # 5 корутин на все файлы.

  Для парсинга аргументов необходимо использовать стандартную
  функцию getopt().

  Каждая корутина должна работать вот так: если есть еще не
  отсортированные файлы, то взять один, отсортировать, повторить.
  Если все файлы отсортированы, то корутина завершается.
...
этот бонус) и количество корутин (если делаете этот бонус).
